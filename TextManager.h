...
public:
	...
	const std::shared_ptr<Character> addCharacter(const char& c, const sf::Font& font);

	const void setFont(sf::Font& Font);
	const void setSize(int Size);
	const void setColour(uint8_t Red, uint8_t Green, uint8_t Blue, uint8_t Alpha = 255);
